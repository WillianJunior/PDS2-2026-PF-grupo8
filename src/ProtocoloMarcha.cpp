...
void ProtocoloMarcha::quedaEnergia(const std::vector<std::shared_ptr<Maquina>>& maquinas) {
    for (size_t i = 0; i < maquinas.size(); i++) {
        if (!maquinas[i]) {throw std::runtime_error("Erro de sincronia: Maquina nula encontrada ao iniciar protocolo de queda de energia.");}
...
void ProtocoloMarcha::modoNormal(const std::vector<std::shared_ptr<Maquina>>& maquinas) {
    for (size_t i = 0; i < maquinas.size(); i++) {
        if (!maquinas[i]) {throw std::runtime_error("Erro de sincronia: Maquina nula encontrada ao mudar para modo normal.");}
